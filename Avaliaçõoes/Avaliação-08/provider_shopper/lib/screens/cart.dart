...
class MyCart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        color: const Color.fromARGB(255, 51, 196, 253),
        child: Column(
          children: [
            Expanded(
              child: Padding(
          ...
          children: [
            ...
            FilledButton(
              onPressed: () {
                ...
                String productDetails = cart.items
                    .map((item) => '${item.name} - \$${item.price}')
                    .join(' + ');

                String mMessage =
                    '$productDetails = Ptotal \$${cart.totalPrice} - Lucas Venicio';

                // Show SnackBar with the custom message
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(mMessage)),
                );
              },
